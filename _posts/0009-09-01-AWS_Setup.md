...
* **NOTE:** All tools should be installed locally (e.g., /home/ubuntu/bin/) in a different location from where students will install tools in their exercises.

* Paths to pre-installed tools can be added to the .bashrc file. It may also be convenient to add `export RNA_HOME=~/workspace/rnaseq` to the .bashrc file. See <http://genomedata.org/rnaseq-tutorial/bashrc_copy>.
...
* cbw-rna-2023 (ami-04a3ec4627540529c; N. Virginia)
...
### Creating a docker image from the entire AMI
NOTE: You can create a Docker image of the entire system representing an AMI to allow even more portability of the environment.  Essentially you make a snapshot of the boot drive, copy it to a new volume, mount that volume as a second drive, tar the whole thing and feed it to docker import.

An example set of detailed instructions on how to do this can be found here.
