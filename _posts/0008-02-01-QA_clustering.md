...
```R
list.files("data/single_cell_rna/cellranger_outputs") # use this command or naviagate to the file in the finder pane
```
...
sample.data  = list() # a list to hold all our seurat object
...
Let's get a feel for what this seurat object looks like and how we can access the information we will use for quality assessemnt, clustering,a nd further analysis.
...
The raw counts are stores in an assay called `RNA`. 
...
Gm26901       . . . . . . . . . . . . . . . . . . . . . . . . . . . . .  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . ......
 ...
 ..............................
```
The counts matrix is not very human readable and not at all human interperatable. Again, if we could see the whole matric it would look like a table with the columns as cell barcodes and the row names as genes. The numbers are how many reads of that gene were observed in each cell.

We store our human readable annotations in a place called `meta.data`. We can see that this is much more interpretable with row names as cell barcodes and the columns as labels for each cell. As we analyze our data we will add a lot of columns to `meta.data` so get use to looking at it! 
Automatically `meta.data` is populated with three columns:
 1. orig.ident: the sample this cell came from
 2. nCount_RNA: the number of reads capturing the cell
...
Note that there is actually two ways to access the `meta.data` information:
```R
# viewing meta.data
head(Rep1_ICB_data_seurat_obj@meta.data)
...
We can view the cell's identites by using the `Idents` function. Our cell's identites are set to be the `orig.ident` column in the `meta.data`. When when use our table function to create a summary, we see that all cells are from the same sample, which is expected since we are just looking at one sample's seurat object right now. 
...
It is very easy to add a column to `meta.data`. Let's start by adding the percentage of mitocondrial genes, this will be a useful number for when we begin removing low quality cells. We will call our column `percent.mt` and will use the function `PercentageFeatureSet` to calculate the percentage of all the counts belonging to a subset of the possible features for each cell. We will use the pattern arugment to match all genes that start with mt (the carrot `^` character means "starts with") and specfify that we want this calculation to be done on our RNA assay. 
...
We will use `pecent.mt` to assess the quality if our cells, lets take a quick look at this one samples data before moving on to process all our samples. Using violen plots we take a look at the number of reads, the number of genes, and the percentage of mitocondiral genes in each cell. 
...
p
```
We see that a majority of cells have greater than 1000 genes and a mitocondrial percentage of less 12. Cells that fall outside of this range are most likely dead or of low quality, we will want to filter these out.

![rep1_icb prefiltered violen plot](/assets/module_8/rep1_icb_prefiltered.vlnplot.png)

#### Creating Seurat Objects for all Samples

The above code accomplishes what we want to do just find but it requires alot of paths and sample names to be exactly correct for it to work. There is a lot room for small mistakes to be made and it would take a long time to write out all the commands. 

**Can you write a for-loop to read in all six sample files?** 

Our for-loop will accomplish two major steps. The first is to read in the cellranger files and the second is to create seurat object. So this for-loop can be read: "For every sample in the sample_names list, paste together a path where the h5 file for that sample can be found, print that path for verification, use the function `Read10X_h5` to read the h5 file, create a seurat object using the function `CreateSeuratObject`, and finally add that sample's seurat object to a list of seurat objects. When the for-loop completes, our `sample.data` list should contain all 6 samples. 
...
As we did for a single sample, we want to calculate percentage of mitochondrial genes for each cell. We wil again use a for-loop to accomplish this for all samples:
...
Let's say we decide to remove all cells with gene counts greater than 1000 and mitochondrial percetnages less than 12. 

We will mark cells that we want to keep. This `ifelse` function statment read: if `percent.mt` is less that or equal to 12 we will mark it as TRUE to keep it, otherwise we will mark it as FALSE to filter it out.
...
We will do the same for the number if genes.
...
Let's see how many cells we will have after filter:
```R
for (sample in sample_names) {
  print(sample)
...
Now that we have assessed the quality on a per-sample level we can merge all our samples together to create one seurat object. The `merge` function will accomplish this for us and add the sample name to end of the cell barcodes in the case that there are suplicate barcodes across samples.
...
Let's now filter out merged object with the cutoffs deceided above. 
...
Cell Cycle scoring is another step which can add extremely useful information to your seurat object. We will download a csv list of genes whihc are associated with different phases of the the cell cycle. The seurat function 'CellCycleScoring' will then calculate a score for each cell based on the given genes and also assign the cell a phase. This function will add the columns S.Score, G2M.Score, and Phase to the `meta.data`.

`CellCycleTiroshSymbol2ID.csv` looks like:

```
List  Gene Symbol Ensembl Gene ID
G2/M  ANLN  ENSG00000011426
G2/M  ANP32E  ENSG00000143401
G1/S  ATAD2 ENSG00000156802
G2/M  AURKA ENSG00000087586
G2/M  AURKB ENSG00000178999
G2/M  BIRC5 ENSG00000089685
G1/S  BLM ENSG00000197299
G1/S  BRIP1 ENSG00000136492
G2/M  BUB1  ENSG00000169679
G1/S  CASP8AP2  ENSG00000118412
